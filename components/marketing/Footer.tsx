...
export default function Footer() {
  return (
    <footer className="border-t border-white/[0.06] bg-navy-deep">
      <div className="mx-auto max-w-7xl px-5 pb-10 pt-16 lg:px-8">
        ...
        <div className="mb-14 grid grid-cols-2 gap-8 md:grid-cols-5 md:gap-10">
            ...
            <div className="mt-6 flex items-center gap-3">
              {socials.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/[0.07] bg-white/[0.05] text-white/35 transition-all duration-150 hover:bg-white/[0.09] hover:text-white/70"
                >
                  {s.icon}
                </a>
              ))}
          ...
          {sitemap.map((col) => (
            <div key={col.heading} className="col-span-1">
              <p className="mb-4 text-[11px] font-bold uppercase tracking-[0.1em] text-white/35">
                {col.heading}
              </p>
              <ul className="flex flex-col gap-2.5">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-white/50 transition-colors duration-150 hover:text-white/85"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="flex flex-col items-center justify-between gap-3 border-t border-white/[0.06] pt-6 sm:flex-row">
          <p className="text-xs text-white/25">
            &copy; 2026 Paylio by ApexCode. All rights reserved.
          </p>
          <p className="text-xs text-white/20">
            Paylio is a portfolio concept and does not offer real financial services.
          </p>
        </div>
      </div>
    </footer>
