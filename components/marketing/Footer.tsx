    ...
    <footer className="bg-navy-deep border-t border-white/[0.06]">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 pt-16 pb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-10 mb-14">
            ...
            <div className="flex items-center gap-3 mt-6">
              {socials.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="w-8 h-8 rounded-lg bg-white/[0.05] border border-white/[0.07] flex items-center justify-center text-white/35 hover:text-white/70 hover:bg-white/[0.09] transition-all duration-150"
                >
                  {s.icon}
                </a>
              ))}
          ...
          {sitemap.map((col) => (
            <div key={col.heading}>
              <p className="text-[11px] font-bold text-white/35 uppercase tracking-[0.1em] mb-4">
                {col.heading}
              </p>
              <ul className="flex flex-col gap-2.5">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-white/50 hover:text-white/85 transition-colors duration-150"
                    >
                      {link.label}
                    </Link>
                  </li>
