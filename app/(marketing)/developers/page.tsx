...
export default function DevelopersPage() {
  return (
    <div>
          ...
          <div className="grid grid-cols-1 items-start gap-16 lg:grid-cols-2">
            <div className="pt-4">
              ...
              <p className="mb-8 max-w-lg text-lg leading-relaxed text-white/50">
                The Paylio API is RESTful, predictable, and built for developers who value clarity
                over clever. One SDK, full coverage.
              </p>
            ...
            {/* Code sample */}
            <div className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0d1117] shadow-2xl">
              <div className="flex items-center gap-2 border-b border-white/[0.07] bg-[#161b22] px-4 py-3">
        ...
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <div className="overflow-hidden rounded-2xl border border-white/[0.07]">
            {endpoints.map((ep, i) => (
              <div
                key={ep.path}
                className={`flex flex-col gap-3 p-5 transition-colors hover:bg-white/[0.02] sm:flex-row sm:items-center ${i < endpoints.length - 1 ? 'border-b border-white/[0.06]' : ''}`}
              >
                <span
                  className={`w-14 flex-shrink-0 rounded-md px-2.5 py-1 text-center text-xs font-black ${ep.method === 'POST' ? 'bg-teal/15 text-teal' : 'bg-blue-400/15 text-blue-400'}`}
                >
                  {ep.method}
                </span>
      ...
      {/* SDKs */}
      <section id="sdks" className="border-t border-white/[0.05] py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <div className="mb-12">
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.15em] text-teal/70">SDKs</p>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {sdks.map((sdk) => (
              <div
                key={sdk.name}
                className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-6 transition-all duration-200 hover:border-teal/20"
              >
                <p className="mb-3 text-3xl">{sdk.icon}</p>
                <p className="mb-3 text-base font-bold text-white">{sdk.name}</p>
                <code className="rounded bg-teal/[0.07] px-2 py-1 text-xs text-teal/70">
                  {sdk.install}
                </code>
              </div>
            ))}
      ...
      {/* Sandbox */}
      <section id="sandbox" className="border-t border-white/[0.05] py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2">
            <div>
              ...
              <p className="mb-8 leading-relaxed text-white/50">
                The sandbox environment mirrors production exactly. Every endpoint, every webhook,
                every error code. Flip one environment variable when you are ready to go live.
              </p>
            ...
            <div className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0d1117]">
              <div className="flex items-center gap-2 border-b border-white/[0.07] bg-[#161b22] px-4 py-3">
                <span className="h-2.5 w-2.5 rounded-full bg-white/10" />
                <span className="h-2.5 w-2.5 rounded-full bg-white/10" />
                <span className="h-2.5 w-2.5 rounded-full bg-white/10" />
      ...
      {/* Webhooks */}
      <section id="webhooks" className="border-t border-white/[0.05] py-24">
        <div className="mx-auto max-w-4xl px-5 text-center lg:px-8">
          <p className="mb-4 text-xs font-bold uppercase tracking-[0.15em] text-teal/70">
            Webhooks
          </p>
          <h2 className="mb-5 text-4xl font-black tracking-tight">React to events in real time.</h2>
          <p className="mx-auto mb-12 max-w-2xl leading-relaxed text-white/50">
            Paylio sends a signed webhook to your endpoint the moment a transfer completes, a card
            is frozen, or a fraud flag is raised. No polling. No missed events.
          </p>
          <div className="grid grid-cols-1 gap-4 text-left sm:grid-cols-3">
            {[
              ...
              {
                event: 'card.frozen',
                desc: 'Fires when a card is frozen by the user or the system',
              },
            ].map((w) => (
              <div
                key={w.event}
              ...
              >
                <code className="mb-2 block font-mono text-xs text-teal">{w.event}</code>
                <p className="text-xs leading-relaxed text-white/40">{w.desc}</p>
              </div>
            ))}
          </div>
        </div>
